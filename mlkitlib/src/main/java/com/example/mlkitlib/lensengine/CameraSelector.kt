...
class CameraSelector(
    ...
    fun createCamera(): Camera {
        val cameraId = getIdForRequestedCamera(configuration.getCameraFacing())
        if (cameraId == -1) {
            throw IOException("Could not find the requested camera.")
        }
        val camera = Camera.open(cameraId)
        val cameraSize =
            selectSizePair(camera, configuration.previewWidth, configuration.previewHeight)
    ...
    val facing: Int
        get() = configuration.getCameraFacing()
